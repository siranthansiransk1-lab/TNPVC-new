    ...
    <SiteLayout>
          ...
          {pillars.map((pillar) => {
            ...
            return (
              <div key={pillar.title} className="float-card group">
                ...
                <h2 className="mt-5 text-base font-black tracking-tight">{pillar.title}</h2>
                <p className="mt-2 text-xs font-medium leading-relaxed text-muted-foreground">{pillar.description}</p>
              </div>
            );
          })}
        </div>
      ...
      <section className="section-padding border-y border-border/40 bg-surface/50">
        <div className="container float-card">
          <h2 className="text-lg font-black tracking-tight">Contractor advantages in the community</h2>
          <div className="mt-6 grid gap-3 md:grid-cols-2">
            {audienceBenefits.contractors.map((point) => (
              <div key={point} className="flex items-start gap-3 rounded-xl bg-background p-4 border border-border/40 shadow-sm">
                <CheckCircle2 className="mt-0.5 size-4 shrink-0 text-primary" />
                <p className="text-xs font-semibold text-muted-foreground leading-relaxed">{point}</p>
              </div>
            ))}
          </div>
        </div>
